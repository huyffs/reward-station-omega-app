...
export type RawProjectReward = IdString &
	RawSchedulable & {
		issuer_id?: string;
		category?: number;
		country_id?: number;
...
export type ProjectReward = IdString &
	Schedulable & {
		issuer_id?: string;
		category?: number;
		country_id?: number;
...
export function buildReward(data: RAW): ProjectReward {
	return {
		id: data.id,
...
export function useProjectRewaerdStore(projectId: string) {
	return useStore(`/project-rewards/${projectId}`, {}, buildReward);
}
